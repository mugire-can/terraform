...
[![Terraform](https://img.shields.io/badge/Terraform-1.15%2B-844FBA?logo=terraform&logoColor=white)](https://developer.hashicorp.com/terraform)
[![Ansible](https://img.shields.io/badge/Ansible-2.19-EE0000?logo=ansible&logoColor=white)](https://www.ansible.com/)
[![VMware Workstation](https://img.shields.io/badge/VMware-Workstation%20Pro-607078?logo=vmware&logoColor=white)](https://www.vmware.com/products/workstation-pro.html)
...
Infrastructure-as-Code project that uses **Terraform** to deploy and manage
**Debian 12** virtual machines on **VMware Workstation Pro**, including a
small two-VM cybersecurity lab (attacker / victim scenario).
...
- [Terraform commands used](#terraform-commands-used)
- [Variables](#variables)
- [Lifecycle management](#lifecycle-management)
- [Cybersecurity lab scenario](#cybersecurity-lab-scenario)
- [Going further: Ansible integration](#going-further-ansible-integration)
...
This project automates the deployment of Debian 12 virtual machines using
Terraform as the Infrastructure-as-Code (IaC) tool, with
[VMware Workstation Pro](https://www.vmware.com/products/workstation-pro.html)
as the hypervisor. It demonstrates:
...
Both VMs are clones of a single prepared template (`Debian12-Base`), so they
automatically share the same virtual network configuration as the template.
...
- `vmrest.exe` (shipped with VMware Workstation) configured with a
  username/password, used by Terraform to talk to VMware Workstation

## Environment setup

1. **Create the Debian 12 template VM** in VMware Workstation (SSH, Open-VM-Tools,
   sudo user, no GUI). Note the full path to its `.vmx` file — this is the
   template Terraform will clone from.

2. **Configure and start the VMware REST API**:
   ```powershell
...
3. **Find the template's VM id** via the REST API (needed in `main.tf`):
   ```powershell
   curl -u <user>:<password> http://127.0.0.1:8697/api/vms
   ```

4. **Copy `terraform.tfvars.example` to `terraform.tfvars`** and fill in your
   real credentials and IP addresses (this file is git-ignored).
...
├── provider.tf                 # vmworkstation provider config
├── variables.tf                # all input variable declarations
├── main.tf                     # kali-like-vm + victim-vm resources
├── provisioners.tf             # remote-exec provisioners
├── inventory.tf                # generates ansible/inventory.ini
├── terraform.tfvars.example    # safe-to-commit template
├── terraform.tfvars            # (git-ignored) real credentials
├── .terraform.lock.hcl         # provider version lock
└── ansible/
    ├── ansible.cfg
    ├── playbook.yml            # configures attacker + victim VMs
    └── inventory.ini           # (git-ignored) generated by Terraform
```
...
> `-parallelism=1` is used throughout this project because the `vmrest`
> REST API does not reliably handle concurrent requests — running actions
> one at a time avoids random API errors.
...
Instead of hardcoding values, the following are parameterized through
...
This was validated by changing `vm_memory` from `1024` to `2048` and
`vm_processors` from `1` to `2` in `terraform.tfvars`, then running
`terraform plan` / `apply`: Terraform correctly detected an **in-place
update** (no VM re-creation), and the change was confirmed inside the guest
with `free -h` and `nproc`.

## Lifecycle management

- **Variables**: see above.
- **Change management**: a memory/CPU change was applied to a running VM
  and Terraform performed an in-place update (`~ update in-place`), not a
  destroy/recreate, keeping the VM's identity and data intact.
- **Destruction**: `terraform destroy -parallelism=1` was used to remove
  the VM entirely, both from the VMware Workstation registry and from disk
  (the `.vmx`/`.vmdk` files were deleted along with the folder).

## Cybersecurity lab scenario

Two VMs are cloned from the same Debian 12 template:

- **`kali-like-vm`** (attacker): Debian 12 with `nmap` and `masscan`
  installed via a `remote-exec` provisioner.
  > Note: this is **not** a real Kali Linux image — a genuine Kali base
  > template would be required for that. Here it is a Debian 12 VM equipped
  > with common reconnaissance tools.
- **`victim-vm`** (victim): Debian 12 running a simple vulnerable HTTP
  service (`python3 -m http.server 8080`), started via a `remote-exec`
  provisioner.

Both VMs inherit the same virtual network configuration as their template
(VMware NAT / VMnet8), so they can reach each other directly.
...
This confirms that the attacker VM can scan and successfully interact with
the vulnerable service exposed by the victim VM, entirely provisioned
through Terraform.

## Going further: Ansible integration

As a further improvement over plain `remote-exec` provisioners, this project
also integrates **Ansible** to configure the lab VMs, which is easier to
extend, re-run idempotently, and maintain than inline shell commands.

Terraform is responsible for **provisioning** (creating the VMs and their
network), while Ansible is responsible for **configuration** (installing
packages, deploying a systemd service). This separation of concerns is a
common and recommended IaC pattern.

### How it works

1. A `local_file` Terraform resource (`inventory.tf`) automatically
   generates `ansible/inventory.ini` from the current `kali_ip` / `victim_ip`
   / `ssh_user` / `ssh_password` variables, right after the VMs are created:
...
2. `ansible/playbook.yml` then configures each host group:
   - **attacker**: installs `nmap` and `masscan` via the `apt` module
   - **victim**: deploys the vulnerable HTTP service as a proper
     **systemd unit** (`victim-http.service`) instead of a background
     `nohup` process, so it survives reboots and is managed like a real
     service (`systemctl status victim-http`)
...
> Note: if running Ansible from WSL against a project folder mounted from
> Windows (`/mnt/d/...`), Ansible may ignore `ansible.cfg` because the
> directory is reported as world-writable. Passing `-i inventory.ini`
> explicitly avoids relying on `ansible.cfg` for the inventory path.

> Requires `sshpass` on the control machine (used by Ansible for
> password-based SSH, e.g. via WSL: `sudo apt install sshpass`), and
> `ansible-core` installed (`pip install ansible`).

This does not replace the `remote-exec` provisioners already validated in
`provisioners.tf` (kept as-is since they were verified to work end-to-end);
it demonstrates an alternative, more maintainable configuration path
suitable for evolving the lab further (e.g. adding more tools, more victim
...
Make sure the target VM's console tab is closed in VMware Workstation
beforehand to avoid the "virtual machine has been locked" error described
above.

## Knowledge base

- [terraform-provider-vmworkstation (elsudano)](https://github.com/elsudano/terraform-provider-vmworkstation)
